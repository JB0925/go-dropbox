...
```
curl -kv -X POST http://localhost:8080/signup -H 'content-type: application/json' -d '{"username": "jesseb", "password": "foobar"}'
```

## Login - returns a JWT
```
curl -kv -X POST http://localhost:8080/login -H 'content-type: application/json' -d '{"username": "jesseb", "password": "foobar"}'
```
...
```
curl -kv -X POST http://localhost:8080/files/sharing -H 'content-type: application/json' -d '{"name": "main.go", "project_name": "foo", "project_id": 6}' -H 'Authorization: '
```
...
to download it:
```
curl -v -X GET http://localhost:8080/files/shared -H 'X-GO-DROPBOX-SHARED-HASH: 5ca07cfe530106ffa949c2ac793fcf240a98034d5b42eb3358968f5343745b55' -H 'X-GO-DROPBOX-SHARER: jesseb' -o ./myfile.png
```
